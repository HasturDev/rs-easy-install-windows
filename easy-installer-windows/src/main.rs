...
fn guide_msys2_installation() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if !Path::new("C:\\msys64\\usr\\bin\\bash.exe").exists() {
        return Err(
            "MSYS2 installation failed. Please try manual installation from https://www.msys2.org/"
                .into(),
        );
    }
...
fn download_and_install_msys2() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if !download_output.status.success() {
        ...
        return Err(format!("Failed to download MSYS2 installer: {}", error_msg).into());
    }
...
fn initialize_msys2() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if !Path::new(msys2_bash).exists() {
        return Err(
            "MSYS2 bash not found after installation. Installation may be incomplete.".into(),
        );
    }
    ...
    let verification_commands = ["pacman --version", "gcc --version"];

    println!("   Verifying installation...");
    for cmd in &verification_commands {
        let output = Command::new(msys2_bash).args(&["-l", "-c", cmd]).output();
        ...
        match output {
            ...
            _ => {
                println!(
                    "     ⚠️  {} not yet available (will install with toolchain)",
                    cmd.split_whitespace().next().unwrap()
                );
            }
        }
...
fn install_gnu_toolchain() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (description, cmd) in &install_commands {
        ...
        if !output.status.success() {
            ...
            {
                println!(
                    "⚠️  {} - skipped (dependency issue or already installed)",
                    description
                );
    ...
    match Command::new("rustup").arg("--version").output() {
        Ok(_) => {
            ...
            if output.status.success() {
                ...
                eprintln!(
                    "❌ Failed to add GNU target: {}",
                    String::from_utf8_lossy(&output.stderr)
                );
            }
        }
        Err(_) => {
            println!("rustup not found. Installing Rust...");
            println!();
            println!("Please follow these steps:");
            println!("1. Visit: https://rustup.rs/");
            println!("2. Download rustup-init.exe");
            println!("3. Run it and select option 2 for custom installation");
            println!("4. Set default host triple to: x86_64-pc-windows-gnu");
            println!("5. Set default toolchain to: stable");
            println!("6. Complete the installation");
            println!();

            print!("Press Enter when Rust installation is complete...");
            io::stdout().flush()?;
            let mut input = String::new();
            io::stdin().read_line(&mut input)?;
        }
...
fn verify_installation() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if compile_output.status.success() {
        ...
        match Command::new("./test_gnu.exe").output() {
            Ok(run_output) => {
                println!("✅ Test program output:");
                println!("{}", String::from_utf8_lossy(&run_output.stdout));
            }
            Err(_) => {
                println!("⚠️  Compiled successfully but couldn't run (may need MSYS2 DLLs in PATH)")
            }
        }
